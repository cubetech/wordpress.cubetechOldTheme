...
class ECF_Panel {
	...
	/* Only posts of these formats will have the panel */
	var $post_formats = array();
	...
	function _attach() {
		...
		if ($this->post_type=='page' && !empty($this->parent_page_path)) {
			...
			add_action('admin_footer', array(&$this, '_print_pages_js'));
		} elseif ($this->post_type=='post' && !empty($this->post_formats)) {
			add_action('admin_footer', array(&$this, '_print_pages_js'));
	...
	function add_fields($fields) {
		foreach ($fields as $field) {
			...
			if (is_a($field, 'ECF_FieldChooseSidebar')) {
				$field->setup_sidebars();
			}
	...
	function save($post_id) {
		...
		// Do not save panel that's not associated with the current post type
