...
/*abstract*/ class base_wp_option {
	// Name of the option, as it will be inserted in database
	var $name;
	
	// Label of the option, as it will be shown on settings page
	var $label;
	
	// Type of the option - e.g. text, textarea etc. Every option type is
	...
	/* final */ function base_wp_option($name, $label) {
		$this->name = $name;
		$this->label = $label;
		$this->id = preg_replace('~^\d*~', '', md5(microtime(true) . $name));
	    $this->init();
	    if (defined('WP_ADMIN')) {
	    	$this->admin_init();
	    }
...
/*abstract*/ class wp_option extends base_wp_option {
	...
	function render($field_html, $colspan = false) {
		if (!$colspan) {
			$html = '
				<tr valign="top" class="field-' . $this->name . '">
			...
			$html = '
				<tr valign="top">
					<th colspan="2" scope="row" class="field-label"><h3>' . $this->label . '</h3>' . $field_html . '</th>
				</tr>
			';
		}
		return $html;
	...
	function save() {
		$this->value = stripslashes_deep($this->value);
...
class wp_option_text extends wp_option {
	...
	function render() {
		$field_html = '<input type="text" name="' . $this->name . '" value="' . htmlentities($this->value) . '" id="' . $this->name . '" ' . $this->get_custom_attrs() . ' />';
...
class wp_option_image extends wp_option_file {
	function admin_init() {
	    ...
	    wp_enqueue_style('thickbox', get_option('home') . '/wp-includes/js/thickbox/thickbox.css');
...
class wp_option_rich_text extends wp_option_textarea {
	...
	function render() {
		...
		if (floatval($wp_version) >= 3.3) {
			wp_editor($this->value, $this->name, array('textarea_rows'=>$this->rows));
		} else {
...
class wp_option_separator extends wp_option {
	function render() {
		$html = '<span class="description">' . $this->help_text . '</span>';
		return wp_option::render($html, true);
	}
	function set_value_from_input() {
		return;
	}
...
$wp_option_set__drag_js_printed = false;
class wp_option_set extends wp_option_text {
	...
	function render() {
		...
		if ($this->draggable) {
			$html = $this->render_as_draggable($current_values);
			return wp_option::render($html);
		}
	...
	function render_drag_js() {
		global $wp_option_set__drag_js_printed;
		if ($wp_option_set__drag_js_printed == true) {
			return;
		}
		$wp_option_set__drag_js_printed = true;
		?>
		<script type="text/javascript" charset="utf-8">
		(function ($) {
			$(function () {
				$('.sort-line').parent().sortable({
					axis: 'y',
					cursor: 'move'
				});
			})
	...
	function render_as_draggable($current_values) {
		$nice_choices = array();
		if (!empty($current_values)) {
			$sliced_choices = array_flip($this->choices);
			foreach ($current_values as $val) {
	    ...
	    <?php $loopID = 0; foreach ($nice_choices as $key => $label) : ?>
	    <li class="sort-line" style="margin: 0px; padding: 3px 0px; line-height: 140%; height: 26px;">
	    	<div style="border: 1px solid #cccccc; padding: 3px; width: 500px; background: #eeeeee; cursor: move;">
		        <input type="checkbox" name="<?php echo $this->name?>[]" <?php echo (isset($current_values[$key])) ? 'checked' : '';?> value="<?php echo $key?>" id="<?php echo $this->name?>_<?php echo $loopID; ?>" />
				<label for="<?php echo $this->name?>_<?php echo $loopID?>"><?php echo $label?></label>
			</div>
	    </li>
...
class wp_option_choose_categories extends wp_option_set {
	function init() {
		$raw_cats = get_categories('child_of=0&parent=0');
...
class wp_option_choose_mixed extends wp_option_set {
	function init() {
		$raw_pages = get_pages('child_of=0&parent=0');
		$options = array();
		foreach ($raw_pages as $p) {
			$options['page|'.$p->ID] = '[Page] ' . $p->post_title;
		}
		
		$raw_categories = get_categories('child_of=0&parent=0');
		foreach ($raw_categories as $c) {
			$options['category|'.$c->term_id] = '[Category] ' . $c->name;
		}
		$this->add_choices($options);
	    wp_option_set::init();
	}
}

/*
Creates Google maps location theme option
Example usage:
	wp_option::factory('google_map', 'loc', 'Location')
		->set_default_coords($lat=10, $lng=10, $zoom_level=5)
*/

class wp_option_google_map extends wp_option_text {
	var $lat=37.423156, $long=-122.084917, $zoom=14;

	function set_default_coords($lat, $long, $zoom) {
	...
	function render() {
		ob_start();
		include(dirname(__FILE__) . '/field-map.php');
		$c = ob_get_Contents();
		ob_end_clean();
	    return wp_option::render($c);
	}
}

include_once("choose-color-scheme.php");

class wp_option_date extends wp_option_text {
	function init() {
		if (defined('WP_ADMIN') && WP_ADMIN) {
			wp_enqueue_script('jqueryui-datepicker', get_bloginfo('stylesheet_directory') . '/lib/theme-options/tpls/jqueryui/jquery-ui-1.7.3.custom.min.js');
			wp_enqueue_style('jqueryui-datepicker', get_bloginfo('stylesheet_directory') . '/lib/theme-options/tpls/jqueryui/ui-lightness/jquery-ui-1.7.3.custom.css');
			wp_enqueue_script('jqueryui-initiate', get_bloginfo('stylesheet_directory') . '/lib/theme-options/tpls/jqueryui/initiate.js');
		}
	    wp_option_text::init();
	}
	
	function render() {
		$field_html = '<input class="datepicker-me" type="text" name="' . $this->name . '" value="' . $this->value . '" id="' . $this->name . '" ' . $this->get_custom_attrs() . ' />';
	    return wp_option::render($field_html);
	}
}

class wp_option_colorpicker extends wp_option_text {
	function init() {
		if (defined('WP_ADMIN') && WP_ADMIN) {
			wp_enqueue_style('farbtastic', get_bloginfo('stylesheet_directory') . '/lib/theme-options/tpls/farbtastic/farbtastic.css');
			
			wp_enqueue_script('farbtastic-custom', get_bloginfo('stylesheet_directory') . '/lib/theme-options/tpls/farbtastic/farbtastic.js', array('jquery'));
			wp_enqueue_script('farbtastic-theme-options', get_bloginfo('stylesheet_directory') . '/lib/theme-options/tpls/farbtastic/farbtastic-init.js', array('jquery', 'farbtastic'));
		}
	    ...
	    $this->html_attrs = array_merge(
			$this->html_attrs,
			array(
				'class'=>'regular-text colorpicker-value',
				'style'=>'width: 195px',
			)
		);
	}
	...
	function render() {
		$field_html = '<input type="text" name="' . $this->name . '" value="' . htmlentities($this->value) . '" id="' . $this->name . '" ' . $this->get_custom_attrs() . ' /><div class="farbtastic-colorpicker" data-link="' . $this->name . '"></div>';
		return wp_option::render($field_html);
	}
}
?>